...
func (client *PostgresDb) InsertAccount(account model.Account) error {
	commandTag, err := client.dbpool.Exec(context.Background(), "INSERT INTO accounts ("+
		"account_id, alias_id, account_role, ref_account_id, affiliate_level, commission_discount, buy_token_volume, sell_percent, slippage, priority_fee, username, chat_id, enable_mev) "+
		...
		account.BuyTokenVolume, account.SellPercent, account.Slippage, account.PriorityFee, account.Username, account.ChatId, account.EnableMev)
	if err != nil {
...
func (client *PostgresDb) GetAccountsByRef(refAccountId int64) ([]model.Account, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&account.AccountId, &account.AliasId, &account.AccountRole, &account.RefAccountId, &account.AffiliateLevel, &account.CommissionDiscount,
			&account.BuyTokenVolume, &account.SellPercent, &account.Slippage, &account.PriorityFee, &account.Username, &account.ChatId, &account.EnableMev); err != nil {
			return accounts, err
		}
